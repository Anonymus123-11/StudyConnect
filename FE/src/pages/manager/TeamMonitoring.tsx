...
import { mockTeams, getTeamMembers } from '../../services/mock/mockData'
    ...
    return (
      <div className="card mb-3 border-l-4 border-[#FF6B00]">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <div className="flex gap-4 text-sm my-2">
              ...
              <span>✓ {team.tasksCompleted} tasks completed</span>
            </div>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold text-[#FF6B00]">{team.healthScore}%</div>
            <div className={`text-xs font-medium capitalize px-2 py-1 rounded mt-2 ${team.status === 'active' ? 'bg-green-100 text-green-700' : team.status === 'at-risk' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-700'}`}>{team.status}</div>
          </div>
        </div>
        <div className="mt-3 flex gap-2">
          ...
          <button className="px-3 py-1 rounded text-sm border border-red-500 text-red-500 hover:bg-red-50">Delete</button>
        </div>
      </div>
    )
  }
    ...
    <div>
      ...
      <Card className="mb-6">
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div className="mt-1 px-3 py-2 bg-[#FFF4E8] rounded text-[#FF6B00] font-bold">{filteredTeams.length} teams</div>
          </div>
        </div>
      </Card>
      ...
      <div className="mt-6 grid md:grid-cols-3 gap-4">
        ...
        <Card className="border-l-4 border-yellow-500">
          ...
          <p className="text-3xl font-bold text-[#FF6B00]">{teams.filter(t => t.status === 'at-risk').length}</p>
        </Card>
        <Card className="border-l-4 border-blue-500">
          <p className="text-sm text-gray-600">Avg Team Health</p>
          <p className="text-3xl font-bold text-[#FF6B00]">{Math.round(teams.reduce((sum, t) => sum + t.healthScore, 0) / teams.length)}%</p>
        </Card>
      </div>
    </div>
  )
